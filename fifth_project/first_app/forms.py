...
# # widgets == field to html input field
# class contactForm(forms.Form):
#     name = forms.CharField(label="Full Name : ", help_text="Total length must be witin 70 charcters", required=False, widget = forms.Textarea(attrs= {'id' : 'text_area', 'class' : 'class1 class 2', 'placeholder' : 'Enter your name'}))
...
#     CHOICES = [('S', 'SMALL'), ('M', 'Medium'), ('L', 'Larger')]
#     size = forms.ChoiceField(choices= CHOICES, widget = forms.RadioSelect)
#     MEAL= [('p', 'pepperoni'),('M', 'Mashroom'),('B', 'Beef')]
#     pizza = forms.MultipleChoiceField(choices= MEAL, widget=forms.CheckboxSelectMultiple)
#     # pizza = forms.ChoiceField(choices= MEAL, widget=forms.CheckboxSelectMultiple)
...
class StudentData(forms.Form):
    # name = forms.CharField(validators=[validators.MaxLengthValidator(10, message='Enter a name with at max 10 characters')])
    ...
    age = forms.IntegerField(validators=[validators.MaxValueValidator(35, message="age must be maximum 35"), validators.MinValueValidator(24, message="age must be at least 24")])
